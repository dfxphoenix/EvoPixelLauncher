...
// Bind the add cracked account button.
document.getElementById('settingsAddCrackedAccount').onclick = (e) => {
    switchView(getCurrentView(), VIEWS.cracked, 500, 500, () => {
        crackedViewOnCancel = VIEWS.settings
        crackedViewOnSuccess = VIEWS.settings
        crackedCancelEnabled(true)
    })
}
...
function populateAuthAccounts(){
    ...
    authKeys.forEach((val) => {
        ...
        if(acc.type === 'microsoft') {
			...
			crackedAuthAccountStr += accHtml2
		}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Download from Launcher Page<span style="font-size: 10px;color: gray;text-shadow: none !important;">Close the launcher and run the dmg to update.</span>', false, () => {
